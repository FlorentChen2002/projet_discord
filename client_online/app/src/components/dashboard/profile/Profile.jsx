...
function Profile({user}) {
    ...
    const otherUser = async(userid) => {
        try {
            const response = await axios.get(`https://projet-discord.onrender.com/api/user/${userid}`,{ withCredentials: true });
    ...
    const getMessage = async(userid) =>{
        try {
            const response = await axios.get('https://projet-discord.onrender.com/api/forum/getallthread',{ withCredentials: true });
            console.log("Requête GET envoyée avec succès :", response.data);
            if (response.status==200){
